...
void CheckAddress(Secp256K1 *T, std::string address, std::string privKeyStr) {
  ...
  // If WIF checksum is invalid, DecodePrivateKey() returns -1.
  // Skip address comparison in self-test to avoid misleading failures.
  if (privKey.IsNegative()) {
    printf("Adress : %s Skipped (invalid private key/WIF)\n", address.c_str());
    return;
  }
...
void Secp256K1::Check() {
  ...
  CheckAddress(this, "15t3Nt1zyMETkHbjJTTshxLnqPzQvAtdCe",
               "5HqoeNmaz17FwZRqn7kCBP1FyJKSe4tt42XZB7426EJ2MVWDeqk");
  CheckAddress(this, "1BoatSLRHtKNngkdXEeobR76b53LETtpyT",
               "5J4XJRyLVgzbXEgh8VNi4qovLzxRftzMd8a18KkdXv4EqAwX3tS");
  CheckAddress(this, "1Test6BNjSJC5qwYXsjwKVLvz7DpfLehy",
               "5HytzR8p5hp8Cfd8jsVFnwMNXMsEW1sssFxMQYqEUjGZN72iLJ2");
  CheckAddress(this, "16S5PAsGZ8VFM1CRGGLqm37XHrp46f6CTn",
               "KxMUSkFhEzt2eJHscv2vNSTnnV2cgAXgL4WDQBTx7Ubd9TZmACAz");
  CheckAddress(this, "1Tst2RwMxZn9cYY5mQhCdJic3JJrK7Fq7",
               "L1vamTpSeK9CgynRpSJZeqvUXf6dJa25sfjb2uvtnhj65R5TymgF");
  CheckAddress(this, "3CyQYcByvcWK8BkYJabBS82yDLNWt6rWSx",
               "KxMUSkFhEzt2eJHscv2vNSTnnV2cgAXgL4WDQBTx7Ubd9TZmACAz");
  CheckAddress(this, "31to1KQe67YjoDfYnwFJThsGeQcFhVDM5Q",
               "KxV2Tx5jeeqLHZ1V9ufNv1doTZBZuAc5eY24e6b27GTkDhYwVad7");
  CheckAddress(this, "bc1q6tqytpg06uhmtnhn9s4f35gkt8yya5a24dptmn",
...
Int Secp256K1::DecodePrivateKey(char *key, bool *compressed) {
  ...
  if (key[0] == '5') {
    ...
    if (privKey.size() != 37) {
      ...
      // Mark invalid using a negative sentinel (top bit set) so callers can
      // reliably detect the error.
      ret.SetInt32(1);
      ret.bits64[NB64BLOCK - 1] = 0x8000000000000000ULL;
      return ret;
    }
    ...
    int count = 31;
    for (int i = 1; i < 33; i++) ret.SetByte(count--, privKey[i]);
    ...
    if (c[0] != privKey[33] || c[1] != privKey[34] || c[2] != privKey[35] || c[3] != privKey[36]) {
      ...
      // Mark invalid (negative sentinel) so callers can bail out.
      ret.SetInt32(1);
      ret.bits64[NB64BLOCK - 1] = 0x8000000000000000ULL;
      return ret;
    }
...
#define KEYBUFFSCRIPT(buff, h)                                                                    \
  ...
  (buff)[3] =                                                                                     \
      (uint32_t)h[10] << 24 | (uint32_t)h[11] << 16 | (uint32_t)h[12] << 8 | (uint32_t)h[13];     \
  (buff)[4] =                                                                                     \
      (uint32_t)h[14] << 24 | (uint32_t)h[15] << 16 | (uint32_t)h[16] << 8 | (uint32_t)h[17];     \
...
void Secp256K1::GetHash160(int type, bool compressed, Point &k0, Point &k1, Point &k2, Point &k3,
...
Point Secp256K1::ParsePublicKeyHex(std::string str, bool &isCompressed) {
  ...
  switch (type) {
    ...
    default:
      printf(
          "ParsePublicKeyHex: Error invalid public key specified (Unexpected prefix (only 02,03 or "
          "04 allowed)\n");
      exit(-1);
  }
...
#define CHECKSUM(buff, A)                                                                         \
  ...
  (buff)[2] =                                                                                     \
      (uint32_t)A[8] << 24 | (uint32_t)A[9] << 16 | (uint32_t)A[10] << 8 | (uint32_t)A[11];       \
  (buff)[3] =                                                                                     \
      (uint32_t)A[12] << 24 | (uint32_t)A[13] << 16 | (uint32_t)A[14] << 8 | (uint32_t)A[15];     \
  (buff)[4] =                                                                                     \
      (uint32_t)A[16] << 24 | (uint32_t)A[17] << 16 | (uint32_t)A[18] << 8 | (uint32_t)A[19];     \
...
bool Secp256K1::CheckPudAddress(std::string address) {
  ...
  return (pubKey[21] == chk[0]) && (pubKey[22] == chk[1]) && (pubKey[23] == chk[2]) &&
         (pubKey[24] == chk[3]);
}
