    ...
    addItem:
      ...
      if (pos < maxFound) {
        ...
        out[pos * ITEM_SIZE32 + 2] =
            (uint32_t)(incr << 16) | (uint32_t)(mode << 15) | (uint32_t)(endo);
...
__device__ __noinline__ void CheckP2SHHash(uint32_t mode, prefix_t *prefix, uint64_t *px,
...
#define CHECK_PREFIX(incr) \
  CheckHash(mode, sPrefix, px, py, j *GRP_SIZE + (incr), lookup32, maxFound, out)

// -----------------------------------------------------------------------------------------

__device__ void ComputeKeys(uint32_t mode, uint64_t *startx, uint64_t *starty, prefix_t *sPrefix,
...
#define CHECK_PREFIX_P2SH(incr) \
  CheckP2SHHash(mode, sPrefix, px, py, j *GRP_SIZE + (incr), lookup32, maxFound, out)
