...
int _ConvertSMVer2Cores(int major, int minor) {
  ...
  sSMtoCores nGpuArchCoresPerSM[] = {
      ...
      {0x89, 128}, // Ada
      {0x90, 128}, // Hopper
      {0xA0, 128}, // Blackwell (assumption)
      {0xA1, 128}, // Blackwell (assumption)
      {-1, -1} };
...
GPUEngine::GPUEngine(int nbThreadGroup, int gpuId, uint32_t maxFound,bool rekey) {
  ...
  // Default-initialize all resources for safe early-returns.
  stream = 0;
  evt = 0;
  inputPrefix = NULL;
  ...
  // Prefer blocking CPU waits (avoids busy-spin in host thread).
  // Must be called before the first context creation on this host thread.
  {
    cudaError_t f = cudaSetDeviceFlags(cudaDeviceScheduleBlockingSync | cudaDeviceMapHost);
    if (f != cudaSuccess && f != cudaErrorSetOnActiveProcess) {
      printf("GPUEngine: cudaSetDeviceFlags: %s\n", cudaGetErrorString(f));
      return;
    }
  }
  ...
  // Dedicated stream for kernel + copies (avoids implicit sync on the legacy default stream)
  err = cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);
  if (err != cudaSuccess) {
    printf("GPUEngine: cudaStreamCreateWithFlags: %s\n", cudaGetErrorString(err));
    return;
  }
  err = cudaEventCreateWithFlags(&evt, cudaEventDisableTiming);
  if (err != cudaSuccess) {
    printf("GPUEngine: cudaEventCreateWithFlags: %s\n", cudaGetErrorString(err));
    return;
  }
...
void GPUEngine::SetPrefix(std::vector<LPREFIX> prefixes, uint32_t totalPrefix) {
  ...
  // Fill device memory (avoid legacy-stream implicit synchronization)
  cudaMemcpyAsync(inputPrefix, inputPrefixPinned, _64K * 2, cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(inputPrefixLookUp, inputPrefixLookUpPinned, (_64K+totalPrefix) * 4, cudaMemcpyHostToDevice, stream);
...
bool GPUEngine::callKernel() {
  ...
  // Reset nbFound
  cudaMemsetAsync(outputPrefix,0,4,stream);

  // Call the kernel (Perform STEP_SIZE keys per thread)
  if (searchType == P2SH) {

    if (hasPattern) {
      comp_keys_p2sh_pattern << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP, 0, stream >> >
        (searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
    } else {
      comp_keys_p2sh << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP, 0, stream >> >
    ...
    if (hasPattern) {
      ...
      comp_keys_pattern << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP, 0, stream >> >
        (searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
    } else {
      //if (searchMode == SEARCH_COMPRESSED) {
	  if (0) {
        comp_keys_comp << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP, 0, stream >> >
          (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
      } else {
        comp_keys << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP, 0, stream >> >
          (searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
      }
    }
...
bool GPUEngine::Launch(std::vector<ITEM> &prefixFound,bool spinWait) {
  ...
  if(spinWait) {
    ...
    cudaMemcpyAsync(outputPrefixPinned, outputPrefix, outputSize, cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);

  } else {

    // Non-busy wait for the first 4 bytes (nbFound)
    cudaMemcpyAsync(outputPrefixPinned, outputPrefix, 4, cudaMemcpyDeviceToHost, stream);
  ...
  // Copy only the produced items (avoid copying full buffer when maxFound is huge)
  cudaMemcpyAsync(outputPrefixPinned, outputPrefix, nbFound*ITEM_SIZE + 4, cudaMemcpyDeviceToHost, stream);
  cudaEventRecord(evt, stream);
  cudaEventSynchronize(evt);
