...
__global__ void comp_keys(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys,
...
__global__ void comp_keys_p2sh(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys,
                               uint32_t maxFound, uint32_t *found) {
  int xPtr = (blockIdx.x * blockDim.x) * 8;
  int yPtr = xPtr + 4 * blockDim.x;
...
__global__ void comp_keys_comp(prefix_t *prefix, uint32_t *lookup32, uint64_t *keys,
                               uint32_t maxFound, uint32_t *found) {
  int xPtr = (blockIdx.x * blockDim.x) * 8;
  int yPtr = xPtr + 4 * blockDim.x;
...
__global__ void comp_keys_pattern(uint32_t mode, prefix_t *pattern, uint64_t *keys,
                                  uint32_t maxFound, uint32_t *found) {
  int xPtr = (blockIdx.x * blockDim.x) * 8;
  int yPtr = xPtr + 4 * blockDim.x;
...
__global__ void comp_keys_p2sh_pattern(uint32_t mode, prefix_t *pattern, uint64_t *keys,
                                       uint32_t maxFound, uint32_t *found) {
  int xPtr = (blockIdx.x * blockDim.x) * 8;
  int yPtr = xPtr + 4 * blockDim.x;
...
GPUEngine::GPUEngine(int nbThreadGroup, int nbThreadPerGroup, int gpuId, uint32_t maxFound,
  ...
  char tmp[512];
  sprintf(tmp, "GPU #%d %s (%dx%d cores) Grid(%dx%d)", gpuId, deviceProp.name,
          deviceProp.multiProcessorCount, _ConvertSMVer2Cores(deviceProp.major, deviceProp.minor),
          nbThread / nbThreadPerGroup, nbThreadPerGroup);
  ...
  */
  ...
  err =
      cudaHostAlloc(&inputPrefixPinned, _64K * 2, cudaHostAllocWriteCombined | cudaHostAllocMapped);
  if (err != cudaSuccess) {
  ...
  err = cudaHostAlloc(&inputKeyPinned, nbThread * 32 * 2,
...
void GPUEngine::PrintCudaInfo() {
  ...
  const char *sComputeMode[] = {"Multiple host threads",
...
void GPUEngine::SetPrefix(std::vector<LPREFIX> prefixes, uint32_t totalPrefix) {
  ...
  // Fill device memory
  cudaMemcpy(inputPrefix, inputPrefixPinned, _64K * 2, cudaMemcpyHostToDevice);
  cudaMemcpy(inputPrefixLookUp, inputPrefixLookUpPinned, (_64K + totalPrefix) * 4,
...
bool GPUEngine::Launch(std::vector<ITEM> &prefixFound, bool spinWait) {
  ...
  if (nbFound > maxFound) {
    ...
    if (!lostWarning) {
      printf(
          "\nWarning, %d items lost\nHint: Search with less prefixes, less threads (-g) or "
          "increase maxFound (-m)\n",
          (nbFound - maxFound));
      lostWarning = true;
    }
...
bool GPUEngine::Check(Secp256K1 *secp) {
  ...
  if (!ripemd160_comp_hash((uint8_t *)(outputPrefixPinned + 5), hc)) {
    printf("\nGetHask160Comp wrong:\n%s\n%s\n",
           toHex((uint8_t *)(outputPrefixPinned + 5), 20).c_str(), toHex(h, 20).c_str());
    return false;
  }
  ...
  if (searchMode == SEARCH_BOTH) {
    printf(
        "Warning, Check function does not support BOTH_MODE, use either compressed or "
        "uncompressed");
    return true;
  }
  ...
  if (!ok) {
    int nbF = nbFoundCPU[0] + nbFoundCPU[1] + nbFoundCPU[2] + nbFoundCPU[3] + nbFoundCPU[4] +
              nbFoundCPU[5];
