...
void checkKeySpace(BITCRACK_PARAM * bc, Int& maxKey) {
	...
	if (bc->ksStart.IsGreater(&maxKey) || bc->ksFinish.IsGreater(&maxKey)) {
		printf("[ERROR] START/END IsGreater %64s \n", maxKey.GetBase16().c_str());
		exit(-1);
	}
...
void loadProgress(string fileName, BITCRACK_PARAM * bc) {
	...
	if (fileName.length() > 0) {
		...
		if (fp != NULL) {
			...
			printf("[load] start=%64s \n", bc->ksStart.GetBase16().c_str());
			printf("[load]  next=%64s \n", bc->ksNext.GetBase16().c_str());
			printf("[load]   end=%64s \n", bc->ksFinish.GetBase16().c_str());
			fclose(fp);
		}
	}
...
int main(int argc, char* argv[]) {
  ...
  if(bc->shareN > 1){
	  //printf("[share][before] start=%64s\n", bc->ksStart.GetBase16().c_str());
	  //printf("[share][before]   end=%64s\n", bc->ksFinish.GetBase16().c_str());
	  ...
	  //printf("[share][after_] start=%64s\n", bc->ksStart.GetBase16().c_str());
	  //printf("[share][after_]   end=%64s\n", bc->ksFinish.GetBase16().c_str());
  }
  ...
  printf("[keyspace] start=%64s\n", bc->ksStart.GetBase16().c_str());
  printf("[keyspace]   end=%64s\n", bc->ksFinish.GetBase16().c_str());
