...
void parseFile(string fileName, vector<string> &lines) {
  ...
  while (getline(inFile, line)) {
    ...
    if (line.length() > 0) {
      ...
      if (loaddingProgress) {
        if ((nbLine % 50000) == 0)
          printf("[Loading input file %5.1f%%]\r",
                 ((double)nbLine * 100.0) / ((double)(nbAddr) * 33.0 / 34.0));
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  printf("VanitySearch (Modern-Stable v1) v" RELEASE "\n");
  ...
  VanitySearch *v =
      new VanitySearch(secp, prefix, seed, searchMode, gpuEnable, stop, outputFile, sse, maxFound,
                       rekey, caseSensitive, startPuKey, paranoiacSeed);
