...
void sha256_checksum(uint8_t *input, int length, uint8_t *checksum) {
  ...
  // Correct double-SHA256 checksum (first 4 bytes)
  // Used by Base58Check (addresses/WIF). Length here is always small (<= 34).
  // The previous Transform2 fast-path produced incorrect results on modern toolchains.
  uint8_t d[32];
  sha256(input, length, d);
  sha256(d, 32, d);
  memcpy(checksum, d, 4);
