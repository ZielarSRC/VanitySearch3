...
static void Transform(uint64_t state[8], const uint8_t buf[128]) {
  ...
  for (i = 16; i < 80; i++) W[i] = W[i - 16] + G0(W[i - 15]) + W[i - 7] + G1(W[i - 2]);
...
void CSHA512::Finalize(unsigned char hash[64]) {
  ...
  for (i = 0; i < 8; i++) WRITEBE64(hash + 8 * i, s[i]);
}
...
void hmac_sha512_init(CSHA512 &ctx, const uint8_t key[SHA512_BLOCK_SIZE]) {
  ...
  // Inner padding
  for (i = 0; i < SHA512_BLOCK_SIZE / 8; i++) pad[i] = keyPtr[i] ^ IPADLL;

  ctx.Initialize();
  ctx.WriteDirect128((unsigned char *)pad);
}

void hmac_sha512_done(CSHA512 &ctx, const uint8_t key[SHA512_BLOCK_SIZE],
  ...
  for (i = 0; i < SHA512_BLOCK_SIZE / 8; i++) pad[i] = keyPtr[i] ^ OPADLL;
...
void pbkdf2_hmac_sha512(uint8_t *out, size_t outlen, const uint8_t *passwd, size_t passlen,
                        const uint8_t *salt, size_t saltlen, uint64_t iter) {
  ...
  for (i = 1; outlen > 0; i++) {
    ...
    for (j = 2; j <= iter; j++) {
      ...
      hmac_sha512_done(hmac, key, U);
      for (k = 0; k < SHA512_HASH_LENGTH / 8; k++) Fptr[k] ^= Uptr[k];
    }
